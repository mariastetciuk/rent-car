  ...
  return (
    <div className={scss.overlay} onClick={handleOverlayClick}>
      <div className={scss.modal}>
        <img src={img} alt={`${make} ${model} ${year}`} width="200px" />
        ...
        <div className={scss.block__wrapper}>
          <h3 className={scss.block__title}>
            Accessories and functionalities:
          </h3>
          <ul className={scss.tags__list}>
            {[...accessories, ...functionalities].map(item => (
              <li className={scss.tags__item} key={item}>
                {item}
              </li>
            ))}
          </ul>
        ...
        <a className={scss.tel} href="tel:+380730000000" tag="a">
          Rental car
        </a>
        <button className={scss.modal__btn} onClick={closeModal}>
          <FiX size="24" />
        </button>
      </div>
    </div>
